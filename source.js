...
const mydata = JSON.parse(fs.readFileSync('config.json'));
const currentelection = mydata['currentelection'];
...
function codeMsg(msg) {
	...
	const sendcode = '```' + code + '```'
	const msg1 = 'https://www.youtube.com/watch?v=dQw4w9WgXcQ'
...
function getDateObject(existed) {
	const data = convertMiliseconds(existed)
	...
	const moredata = calculateTiming(data.d)
...
function timeHandler(existed) {
	const dateobj = getDateObject(existed)
...
function help(msg, canvote) {
	...
	if (canvote) {
		embed.addField('$code', 'Election command. DMs the user the election code, along with the instructions for to vote.')
	}
...
function randomString(length = 64) {
	...
	for (let i = length; i > 0; --i) {
		const num = Math.floor(Math.random() * (126 - 33 + 1) + 33)
...
function readJSON() {
	let data = fs.readFileSync('json/main.json')
	data = JSON.parse(data)
	data.election.users.forEach(function(item) {
		const decrypted = CryptoJS.AES.decrypt(item, process.env.ENCRYPTION_KEY)
...
function saveJSON() {
	...
	users.forEach(function(item) {
		const encrypted = CryptoJS.AES.encrypt(item, process.env.ENCRYPTION_KEY)
...
client.on('guildMemberAdd', member => {
	let existed = Date.now() - member.user.createdAt
	...
	if (msg.content.startsWith('$questioning')) {
		if (msg.member.roles.cache.has('766386531681435678')) {
			...
			if (member.roles.cache.has('766386531681435678')) {
				...
				const rolenames = []
				const roles = member.roles.member._roles
				roles.forEach(function(item) {
					const role = msg.guild.roles.cache.get(item)
